--// SWILL DEX v4 (Stable, Expandable Tree + Voxel Pipette) //
...
local collectedColors = {}  -- для сбора RGB руды
...
treeFrame.AutomaticCanvasSize = "Y"  -- авто‑подгон высоты
...
-- ===================== ДЕРЕВО ОБЪЕКТОВ =====================
local nodes = {}  -- [instance] = Button

-- Функция создания кнопки для объекта
local function createNode(instance, depth)
    ...
    -- При клике — показать инфу и раскрыть/свернуть
    btn.MouseButton1Click:Connect(function()
        ...
        if #children > 0 then
            -- Проверяем, раскрыт ли уже (по наличию хотя бы одного дочернего узла)
            ...
            if expanded then
                -- Сворачиваем: удаляем все кнопки, которые принадлежат потомкам
                ...
                -- Раскрываем: создаём кнопки для всех детей
            ...
            treeFrame.CanvasSize = UDim2.new(0,0,0,0)  -- AutomaticCanvasSize обновит
        end
    end)
...
print("SwillDex v4 готов. Дерево работает, пипетка ловит воксели, F9 – список цветов.")
